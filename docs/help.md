...
网页端创建的搜索订阅不会自动知道要推送到哪个 Telegram chat。处理方式：

1. 在网页端「设置 -> 通知集成 -> 搜索订阅」创建订阅，并勾选 Telegram。
2. 在 Bot 里发送 `/subscriptions`。
3. Bot 会在 `Available saved searches` 下列出网页端可绑定订阅。
4. 发送 `/subscribe <saved_search_id>`，例如 `/subscribe 12`。
5. 再发送 `/subscriptions`，订阅会出现在 `My subscriptions` 下。

`/unsubscribe <subscription_id>` 使用的是 `My subscriptions` 里的订阅 ID，不是网页端 saved search ID。
