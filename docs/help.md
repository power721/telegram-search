...
Bot 设置保存后由后台 runtime 自动生效，无需重启服务。启用后系统会自动注册 Telegram 命令菜单。轮询间隔默认 `3s`，通常不需要调大；如果服务器资源紧张或 Bot 调用频率很低，可以改成 `5s` 或 `10s`。
...
`/subscribe <keyword>` 会创建启用 Telegram 通知的保存搜索，并绑定当前 chat。后续历史同步或实时监听写入新资源并匹配该保存搜索时，系统会通过 Bot 向订阅 chat 推送消息。
同一个 chat 对同一个资源只会推送一次；如果多个搜索订阅同时命中同一资源，Bot 会自动跳过重复消息。

### 网页端绑定 Telegram 接收人

网页端可以直接给 Bot 订阅，但 Telegram 不允许服务端凭用户名主动推送，系统必须先拿到 `chat_id`。`chat_id` 会在用户或群组给 Bot 发送消息时自动记录。

推荐流程：

1. 打开 Bot，发送 `/start`。
2. 如果要推送到群组，把 Bot 拉进群并在群里发送 `/start`。
3. 打开管理后台「设置 -> 通知集成 -> 搜索订阅」。
4. 勾选 Telegram。
5. 在「Telegram 接收人」下拉框选择已出现的用户或群组。
6. 保存搜索订阅。

也可以在 Bot 里绑定网页端已有订阅：

1. 在 Bot 里发送 `/subscriptions`。
2. `Available saved searches` 会列出可绑定的网页订阅。
3. 发送 `/subscribe <saved_search_id>`，例如 `/subscribe 12`。
4. 再发送 `/subscriptions`，订阅会出现在 `My subscriptions` 下。

`/subscriptions` 里 `sub #` 是 Bot 订阅记录 ID，`saved #` 是网页端搜索订阅 ID。`/unsubscribe <id>` 优先按 `sub #` 解绑；找不到时会按 `saved #` 解绑当前 chat。
