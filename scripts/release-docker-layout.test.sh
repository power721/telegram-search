...
LOCAL_RUNTIME_DOCKERFILE="$ROOT_DIR/Dockerfile.local"
...
assert_file_contains "$LOCAL_RUNTIME_DOCKERFILE" "COPY --chmod=755 dist/local/tg-search /usr/local/bin/tg-search"
assert_file_contains "$LOCAL_RUNTIME_DOCKERFILE" "COPY docker/config.yaml /app/config.yaml"
